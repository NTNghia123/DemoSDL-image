...
void Game::initGame() {
	...
	iceChunk  = loadSound("assets\\ice.wav");
    ...
    backButtonPushed.texture = loadTexture("img\\back_button_pushed.png",renderer);
    ...
    SDL_QueryTexture(backButtonPushed.texture, NULL, NULL, &backButtonPushed.w, &backButtonPushed.h);
    yesButton.getPos(163,292);
    noButton.getPos(470,292);
    backButton.getPos(0,0);
    backButtonPushed.getPos(0,-3);

    highscoreMenu = loadTexture("img\\high_score.png",renderer);
    infoMenu = loadTexture("img\\info_menu.png",renderer);
    ...
    chooseClassMultiplayer.texture = loadTexture("img\\multiplayer_button.png",renderer);
    SDL_QueryTexture(chooseClassMultiplayer.texture, NULL, NULL, &chooseClassMultiplayer.w, &chooseClassMultiplayer.h);
    chooseClassMultiplayer.getPos(264,458);
...
void Game::play(){
    ...
    while( !quit ) {
        if ( tower.currentFrame == 1 ) {
            if ( status == PLAY_GAME_WIZARD || status == PLAY_GAME_MULTIPLAYER ){
                wizard.x += 60;
        ...
        case INFO:
            prepareScene(renderer);
            renderTexture(infoMenu,0,0,renderer);
            renderTexture(backButton.texture,0,0,renderer);
            getIfInfo();
            presentScene(renderer);
            break;
        ...
        while (SDL_PollEvent(&event))
        {
            switch (event.type)
            {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    renderTexture(backButtonPushed.texture,backButtonPushed.x,backButtonPushed.y,renderer);
                    play(clickSound);
                    status = MENU;
                    }
                default:
        ...
        while (SDL_PollEvent(&event))
        {
            switch (event.type)
            {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    archer.x = INIT_X;
                    archer.y = INIT_Y;
                    status = PLAY_GAME_ARCHER;
                    prevClass = ARCHER;
                    std::cerr << "archer" << std::endl;
                    ...
                    renderTexture(chooseClassMultiplayer.texture,chooseClassMultiplayer.x,chooseClassMultiplayer.y,renderer);
        ...
        while (SDL_PollEvent(&event))
        {
            switch (event.type)
            {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (isInRect(yesButton)){
                    play(clickSound);
                    if (prevClass == ARCHER) status = PLAY_GAME_ARCHER;
            ...
            switch (event.type)
            {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (isInRect(helpButton)){
                        ...
                        if ( status == PLAY_GAME_ARCHER || status == PLAY_GAME_MULTIPLAYER) ishelpArcher = true;
                    }
                    break;
                default:
                    break;
    ...
    void Game::updateEvent(){
        ...
        if (keyboard[SDL_SCANCODE_SPACE] && manaPercentArcher == 100 ) {
            manaPercentArcher = 0;
            upmanaPercentArcher = 0;
            GameshootUlti();
            archer.shootArrow();
    ...
    void Game::playArcher(){
        ...
        int ww = 140 * manaPercentArcher / 100;
        int hh = 16;
        SDL_Rect rect = { 45, 75, ww, hh };
        SDL_SetRenderDrawColor(renderer, 0, 71, 171, 0); // xanh cobalt
        SDL_RenderFillRect(renderer, &rect);
        if (manaPercentArcher != 100 && upmanaPercentArcher == 2){
            manaPercentArcher += 1;
            upmanaPercentArcher = 0;
        }
        if (upmanaPercentArcher != 2) upmanaPercentArcher++;
    ...
    if (!anyEventProcessed && !wizard.isCasting && wizard.frameCasting == 6 && wizard.health != 0) {
    ...
    void Game::playWizard(){
        ...
        int ww = 140 * manaPercentWizard / 100;
        int hh = 16;
        SDL_Rect rect = { 45, 75, ww, hh };
        SDL_SetRenderDrawColor(renderer, 153, 102, 204, 0); // amethyst
        SDL_RenderFillRect(renderer, &rect);
        if (manaPercentWizard != 100 && upmanaPercentWizard == 2){
            manaPercentWizard += 1;
            upmanaPercentWizard = 0;
        }
        if (upmanaPercentWizard != 2) upmanaPercentWizard++;
    ...
    void Game::playMultiplayer(){
        ...
        SDL_SetRenderDrawColor(renderer, 153, 102, 204, 0); // amethyst
        SDL_RenderFillRect(renderer, &rect);
        if (manaPercentWizard != 100 && upmanaPercentWizard == 3){
            manaPercentWizard += 2;
            upmanaPercentWizard = 0;
        }
        if (upmanaPercentWizard != 3) upmanaPercentWizard++;
        //
        renderTexture(emptyManaBar,40,90,renderer);
        ww = 140 * manaPercentArcher / 100;
        hh = 16;
        SDL_Rect rectManaArcher = { 45, 97, ww, hh };
        SDL_SetRenderDrawColor(renderer, 0, 71, 171, 0); // xanh cobalt
        SDL_RenderFillRect(renderer, &rectManaArcher);
        if (manaPercentArcher != 100 && upmanaPercentArcher == 2){
            manaPercentArcher += 1;
            upmanaPercentArcher = 0;
        }
    ...
    manaPercentArcher = 100;
    manaPercentWizard = 100;
    upmanaPercentWizard = 0;
    upmanaPercentArcher = 0;
    ...
    staticBG = STATIC_BG_day;
    bg.setTexture(SCROLLING_BG_day);
    if (prevClass == ARCHER ) archer.health = 5;
    if (prevClass == WIZARD){
    wizard.health = 5;
    wizard.currentTexture = 0;
    wizard.frameCasting = 0;
    ...
    if (status == PLAY_GAME_ARCHER || status == PLAY_GAME_WIZARD || status == PLAY_GAME_MULTIPLAYER){
    if (mainMusic != nullptr) Mix_FreeMusic(mainMusic);
    mainMusic = loadMusic("assets\\day_music_real.mp3");
    Mix_HaltMusic();
    play(mainMusic);
